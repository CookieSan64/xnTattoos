...
Citizen.CreateThread(function()
	AddTextEntry("Tattoo Shop", "Tattoo Shop")
...
AddEventHandler('esx:playerLoaded', function(playerData)
	ESX.TriggerServerCallback('xnTattoos:requestPlayerTattoos', function(tattooList)
		if tattooList then
...
AddEventHandler('skinchanger:modelLoaded', function()
	ESX.TriggerServerCallback('xnTattoos:GetPlayerTattoos', function(tattooList)
		if tattooList then
...
function BuyTattoo(collection, name, label, price)
	ESX.TriggerServerCallback('xnTattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("xnTattoos:RemoveTattoo", currentTattoos)
	ESX.ShowNotification("Vous avez supprimé le tatouage ~y~" .. GetLabelText(label) .. "~s~")
end
...
function CreateScale(sType)
	...
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Changer la vue de la caméra", 21, "Modifier l'opacité", {90, 89}, "Acheter/Retirer un tatouage", 191)
		scaleString = sType
	end
end
